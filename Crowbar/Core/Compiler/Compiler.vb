...
#Region "Private Methods in Background Thread"
	...
	Private Function CompilerInputsAreValid() As Boolean
		...
		If TheApp.Settings.CompileOptionMoveMaterialIsChecked Then
			If String.IsNullOrEmpty(TheApp.Settings.CompileOptionMoveMaterialPath) Then
				inputsAreValid = False
				Me.WriteErrorMessage(1, "Selected Material folder is not valid, if don't want to use this, turn off ""Move Material""")
			End If
		End If
			...
			If result = "success" Then
				...
				Else
					If File.Exists(compiledMdlPathFileName) Then
						If TheApp.Settings.CompileOptionMoveMaterialIsChecked Then
							MoveMaterialThroughMdl(compiledMdlPathFileName)
						End If
	...
	Private Sub MoveMaterialThroughMdl(ByVal targetPathFileName As String)
		...
		Dim texture_count As Int32
		Dim texture_offset As Int32
		Dim texturedir_count As Int32
		Dim texturedir_offset As Int32

		Dim textureFileStreamPosition As Long

		Dim textures_name As List(Of String)
		textures_name = New List(Of String)()

		Dim textures_name_missing As List(Of String)
		textures_name_missing = New List(Of String)()

		Dim textures_path As List(Of String)
		textures_path = New List(Of String)()

		Me.UpdateProgress()
		Me.UpdateProgress(0, "Finding materials")
		Try
			FileStream = New FileStream(targetPathFileName, FileMode.Open, FileAccess.Read, FileShare.ReadWrite)
			If FileStream IsNot Nothing Then
				Try
					'NOTE: Important to set System.Text.Encoding.ASCII so that ReadChars() only reads in one byte per Char.
					FileReader = New BinaryReader(FileStream, System.Text.Encoding.ASCII)
					FileReader.BaseStream.Position = 204





					texture_count = FileReader.ReadInt32()
					texture_offset = FileReader.ReadInt32()
					texturedir_count = FileReader.ReadInt32()
					texturedir_offset = FileReader.ReadInt32()
					Me.UpdateProgress(0, "Finding " + texture_count.ToString() + " Materials")
					If texturedir_count <> 0 AndAlso texturedir_offset <> 0 Then
						If texture_count <> 0 AndAlso texture_offset <> 0 Then
							'Get texture
							Dim tempbuffer As String
							For index As Int32 = 1 To texture_count
								textureFileStreamPosition = texture_offset + (64 * (index - 1))
								FileReader.BaseStream.Position = textureFileStreamPosition
								FileReader.BaseStream.Position = textureFileStreamPosition + FileReader.ReadInt32()
								tempbuffer = FileManager.ReadNullTerminatedString(FileReader)
								textures_name.Add(tempbuffer + ".vtf")
								textures_name.Add(tempbuffer + ".vmt")
							Next
							'Get texturedir 
							For index As Int32 = 1 To texturedir_count
								FileReader.BaseStream.Position = texturedir_offset + (4 * (index - 1))
								FileReader.BaseStream.Position = FileReader.ReadInt32()
								textures_path.Add(FileManager.GetNormalizedPathFileName(FileManager.ReadNullTerminatedString(FileReader)))
							Next
						End If
					End If
		...
		textures_name_missing.AddRange(textures_name)
		...
		For Each texture_path As String In textures_path
			FileManager.CreatePath(Path.Combine(DestFolder, texture_path))
		Next


		For Each texture_name As String In textures_name
			For Each texture_path As String In textures_path
				...
				If File.Exists(Targetfilename) Then
					...
					End Try
					Try
						File.Copy(Targetfilename, Destfilename)
						Me.UpdateProgress(2, "MATERIAL: Copyed material file """ + Targetfilename + """ to """ + Path.Combine(DestFolder, texture_path) + """")
						textures_name_missing.Remove(texture_name)
						CopyCount += 1
					Catch ex As Exception
						Me.UpdateProgress()
						Me.UpdateProgress(2, "WARNING: Crowbar tried to move the file, """ + Targetfilename + """, to """ + Path.Combine(DestFolder, texture_path) + """, but Windows complained with this message: " + ex.Message.Trim())
						Me.UpdateProgress(2, "SOLUTION: Compile the model again (and hope Windows does not complain again) or move the file yourself.")
						Me.UpdateProgress()
					End Try
				End If
		...
		For Each missing_file As String In textures_name_missing
			Me.UpdateProgress(0, "WARNING: Crowbar failed to move the file """ + missing_file + """,is it Exist?")
		Next
		Me.UpdateProgress(1, "MATERIAL: Find " + texture_count.ToString() + " materials and Copy " + CopyCount.ToString() + " Files.")
